...
class Instance(object):
    ...
    def _get_cmd(self, instance_name, ip, model_type="8-64-300", template_type="centos6.4"):
    ...
    def get_idle_ip(self):
        cmd = "select ipaddress,gateway from %s where is_alloc=0 and vlan_id=2" % (self.iptable)
...
# reconfigure host
def reconfigure(instance, name, ip, gateway, flag):
    try:
        # set system type
        yhd_os_type = "centos"
        cmdlist = []
...
def clone(instance, instance_name, template_name, ip, gateway, flag):
    ...
    # reconfigure host
    reconfigure(instance, instance_name, ip, gateway, flag)
...
def main(**kwargs):
    ...
    for i in range(0, kwargs['number']):
        ...
        ip, gateway = instance_db_handle.get_idle_ip()
        if not ip:
        ...
        clone(instance_db_handle, name, template_name, ip, gateway, yhd_flag)
        # ring_point += 1
        # time.sleep(3)
